...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    //val newDf = spark.sql("select joinResult.rectangle as rectangle, count(1)  from joinResult group by joinResult.rectangle order by count(1)" )  
    //val tmp = newDf.coalesce(1)

    val newDf = spark.sql("select rectangle, count(point) from joinResult group by rectangle order by rectangle")
